"""What konnektion asks of a store, and the path rules every one of them is held to.

Three protocols, none of which anything inherits from: what a store has to offer, and the two
things it may offer that konnektion will use if they are there. Keeping the optional halves in
...
The implementations that ship with konnektion are :mod:`konnektion.stores.directory` and
:mod:`konnektion.stores.memory`; the helpers that call through these methods are in
:mod:`konnektion.stores.access`.
"""
...
@runtime_checkable
class KonnektionStore(Protocol):
    """The three operations konnektion needs from a store.
...
class RangeReadable(Protocol):
    ...
    Kept separate from :class:`KonnektionStore` so that a store without it still satisfies the
    protocol konnektion requires -- ``isinstance(store, RangeReadable)`` is then the question
    :func:`konnektion.stores.access.get_range_bytes` asks before deciding whether to fetch a window
    or the whole object.
    """
