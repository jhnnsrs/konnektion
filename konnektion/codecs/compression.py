...
zstd-compressed, which is why raw blobs cost roughly half rather than the multiple their sizes
suggest. ``ZSTD`` is the other value the format defines, and it is a blob-level frame written
*inside* that Parquet column -- which is why the uncompressed length has to be supplied from the
row rather than discovered from the frame.
"""

from __future__ import annotations

from konnektion.errors import FormatError
from konnektion.manifest import COMPRESSION_NONE, COMPRESSION_ZSTD
...
def require_known_compression(compression: str) -> None:
    ...
    if compression not in (COMPRESSION_NONE, COMPRESSION_ZSTD):
        raise FormatError(
            f"`compression` is {compression!r}; the format defines {COMPRESSION_NONE} and "
            f"{COMPRESSION_ZSTD}."
        )
...
def decompress(payload: bytes, compression: str, expected: int) -> bytes:
    ...
    ``expected`` is the uncompressed length, and it is **required rather than discovered**: ZSTD
    frames need not carry their content size, so the format supplies it from the row instead --
    ``6 * node_count`` for positions, ``8 * edge_count`` for edges, ``8 * node_count`` for
    ``UINT64`` node ids. That is what those columns were always for, and it is why the framing
